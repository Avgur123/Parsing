...
public class FieldSetMapperBondEndOfDay implements FieldSetMapper {
    ...
    public Object mapFieldSet(FieldSet fieldSet) throws BindException {
        EndOfDay bond = new EndOfDay();
        Date dateFromFile;

        Map<String, Object> errorModel = new HashMap<>();
        String key = fieldSet.readRawString("secId");
        ...
        try{
            ...
            if (e.getMessage() != "Unparseable number: ") {
        ...
        if (errorCollector.getFullErrorLogMapBondEndOfDate().containsKey(bond.getTradeDate())) {
            if (!errorModel.isEmpty()) {
                errorCollector.getFullErrorLogMapBondEndOfDate().get(bond.getTradeDate()).put(bond.getIsin(), errorModel);
            }
        } else {
            if (!errors.isEmpty()) {
                errorCollector.getFullErrorLogMapBondEndOfDate().put(bond.getTradeDate(), errors);
            }
        }
        return bond;
    }
